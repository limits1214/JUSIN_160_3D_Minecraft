...
HRESULT CUIBreathBarIcon::Initialize(void* pArg)
{
	...
	pDesc->fSizeX = 3.f * MC_UI_SCALE;
	pDesc->fSizeY = 3.f * MC_UI_SCALE;
...

	{
		...
		m_pComOverlayTransform->SetScale(XMVectorSet(9.f * MC_UI_SCALE, 9.f * MC_UI_SCALE, 9.f * MC_UI_SCALE, 0.f));
	}
	return S_OK;
}
...
{
	if (auto pBar = Cast<CUIBreathBar>(GetParentNode()))
	{
		if (pBar->GetRender())
		{
...
HRESULT CUIBreathBarIcon::Render(ID3D11DeviceContext* pContext, const E::RENDER_CTX& ctx)
{
	...
	if (m_eIconType == BREATH_ICON_TYPE::EMPTY)
	{
		{
			E::CB_PER_UI perUI{};
	...
	else if(m_eIconType == BREATH_ICON_TYPE::FULL)
	{
		{
			E::CB_PER_UI perUI{};
			perUI.texIndex = PackTexId(12, 0);
