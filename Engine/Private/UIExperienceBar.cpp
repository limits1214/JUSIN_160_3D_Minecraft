...
void CUIExperienceBar::UpdateGUI()
{
	CUIObject::UpdateGUI();
...
HRESULT CUIExperienceBar::Initialize(void* pArg)
{
...
void CUIExperienceBar::PriorityUpdate(E::_float fTimeDelta)
{
}

void CUIExperienceBar::Update(E::_float fTimeDelta)
{
}

void CUIExperienceBar::LateUpdate(E::_float fTimeDelta)
{
	if (m_bRender)
	{
	...
	// draw 1: BG
	{

		{
			...
			if (SUCCEEDED(pContext->Map(pCb->GetCBuffer().Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource)))
			{
				...
				perUI.texCoord = { 0.f , 64.f / 256.f };
				perUI.uvSize = { 182.f / 256.f, 5.f / 256.f };
...
E::UPtr<CUIExperienceBar> CUIExperienceBar::Create()
{
	auto pInstance = E::ToUPtr(new CUIExperienceBar{});
	if (FAILED(pInstance->InitializePrototype()))
	{
		MSG_BOX("Failed to Created : CUIExperienceBar");
		return nullptr;
	}
	return  pInstance;
}

E::UPtr<E::CPrototype> CUIExperienceBar::Clone(void* pArg)
{
	auto	pInstance = E::ToUPtr(new CUIExperienceBar{ *this });
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CUIExperienceBar");
		return nullptr;
	}
