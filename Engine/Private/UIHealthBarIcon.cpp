...
HRESULT CUIHealthBarIcon::Render(ID3D11DeviceContext* pContext, const E::RENDER_CTX& ctx)
{
	...
	if (m_eIconType == HEALTH_ICON_TYPE::EMPTY)
	{

	}
	else if (m_eIconType == HEALTH_ICON_TYPE::FULL)
	{
		{
			{
				E::CB_PER_UI perUI{};
				perUI.texIndex = PackTexId(12, 0);
				perUI.texCoord = { 52.f / 256.f, 0.f };
				perUI.uvSize = { 9.f / 256.f, 9.f / 256.f };
				if (FAILED(m_pComCBufferPerUI->MapDiscard(pContext, &perUI, sizeof(perUI))))
				{
					return E_FAIL;
	...
	else if (m_eIconType == HEALTH_ICON_TYPE::HALF)
	{
		{
			{
				E::CB_PER_UI perUI{};
				perUI.texIndex = PackTexId(12, 0);
				perUI.texCoord = { 61.f / 256.f, 0.f };
				perUI.uvSize = { 9.f / 256.f, 9.f / 256.f };
				if (FAILED(m_pComCBufferPerUI->MapDiscard(pContext, &perUI, sizeof(perUI))))
				{
					return E_FAIL;
