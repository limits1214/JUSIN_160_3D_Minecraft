...
CCowEntity::CCowEntity(const CCowEntity& rhs)
...
HRESULT CCowEntity::Initialize(void* pArg)
{
    ...
    m_pCenterCollider = CCollBox::Create({ 0.f, 0.49f, 0.f }, { 0.49f, 0.99f, 0.49f });
...
{
    if (m_eCurrentState != COW_STATE::DIE)
    {
        ...
        if (m_TimerAmbientSoundPlay.Get_Finished())
        {
            ...
            if (auto pCam = CGameInstance::Get().GetActiveGameCamera())
            {
                ...
                // 랜덤 재생
                const char* sounds[] = { "COW_SAY_1", "COW_SAY_2" , "COW_SAY_3", "COW_SAY_4" };
    ...
    if (m_eCurrentState == COW_STATE::DIE)
    {
        ...
        E::CGameInstance::Get().AddColliderGroup("Coll_CowCenter", m_pCenterCollider.get());
    ...
    if (m_bIsHit && m_eCurrentState != COW_STATE::FLEE)
    {
        m_eCurrentState = COW_STATE::FLEE;
    ...
    case COW_STATE::WANDER:
        ...
        if (fDistToPlayer < 4.0f) {
            ...
            m_eCurrentState = COW_STATE::IDLE;
            m_fStateTimer = 1.f + (rand() % 30) * 0.1f; // 1~4초 쉬기
        }
        break;

    case COW_STATE::LOOK_AT_PLAYER:
        if (m_bIsHit)
        {
            break;
        }
...
void CCowEntity::VelocityUpdate(E::_float fTimeDelta, _fvector vWishDir)
{
...
void CCowEntity::TakeDamage(uint32_t iDamage, _vector vAttackerPos)
{
    if (m_eCurrentState == COW_STATE::DIE) return; // 이미 죽었거나 피격 쿨타임 중이면 무시
    m_fSpeed = 3.f;
    m_bIsHit = true;
    ...
    if (m_iHeart <= 0)
    {
        ...
        if (auto pCam = CGameInstance::Get().GetActiveGameCamera())
        {
            ...
            // 랜덤 재생
            const char* sounds[] = { "COW_HURT_1", "COW_HURT_2", "COW_HURT_3" };
...
void CCowEntity::ProcessDropDropItem(_float fTimeDelta)
{
}

void CCowEntity::ProcessDestroy(_float fTimeDelta)
{
    ...
    info.eItemType = CItemObject::ITEM_TYPE::ITEM_Raw_Beef;
    info.iCnt = 1;
