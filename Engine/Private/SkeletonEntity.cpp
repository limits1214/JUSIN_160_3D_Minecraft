    ...
    if (m_eCurrentState != SKELETON_STATE::DIE)
    {
        ...
        if (m_TimerAmbientSoundPlay.Get_Finished())
        {
            ...
            if (auto pCam = CGameInstance::Get().GetActiveGameCamera())
            {
                ...
                // 랜덤 재생
                const char* sounds[] = { "SKELETON_SAY_1", "SKELETON_SAY_2" , "SKELETON_SAY_3" };
...
void CSkeletonEntity::TakeDamage(uint32_t iDamage, _vector vAttackerPos)
{
    if (m_eCurrentState == SKELETON_STATE::DIE) return; // 이미 죽었거나 피격 쿨타임 중이면 무시
    m_fSpeed = 3.f;
    m_bIsHit = true;
    ...
    if (m_iHeart <= 0)
    {
        ...
        if (auto pCam = CGameInstance::Get().GetActiveGameCamera())
        {
            ...
            CGameInstance::Get().SoundPlay("SKELETON_DEATH", fVol);
        }
    }
    ...
    if (auto pCam = CGameInstance::Get().GetActiveGameCamera())
    {
        ...
        // 랜덤 재생
        const char* sounds[] = { "SKELETON_HURT_1", "SKELETON_HURT_2", "SKELETON_HURT_3", "SKELETON_HURT_4"};
